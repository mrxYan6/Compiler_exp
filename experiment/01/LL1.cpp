...
struct gramma_token{ 
    ...
    std::map<int, std::set<int>> first, follow;
    ...
    void put_graph() {
        ...
        std::vector<std::vector<int>> biao(not_end.size(), std::vector<int> (end.size(), -1));
        for (auto [l, r] : G) {
            int m = r.size();
            for (int i = 0; i < m; ++i) {
...
int main() {
    ...
    // std::cout << "After remove left recursion:\n";
    std::cout << "\n";
    ...
    for (int i = 0; i < g_token.n; ++i) {
        std::cout << g_token.mp[i] << ": ";
        for (auto x : g_token.first[i]) {
            std::cout << g_token.mp[x] << " ";
        }
        std::cout << std::endl;
    }
    ...
    for (int i = 0; i < g_token.n; ++i) {
        std::cout << g_token.mp[i] << ": ";
        for (auto x : g_token.follow[i]) {
            std::cout << (x == -1 ? "$" :  g_token.mp[x]) << " ";
        }
        std::cout << std::endl;
    }
    ...
    std::cout << "\n";
    std::cout << "SELECT:\n";
    for (auto [l, r] : g_token.G) {
        int m = r.size();
        for (int i = 0; i < m; ++i) {
            std::cout << g_token.mp[l] << " -> ";
            for (auto x : r[i]) {
                std::cout << g_token.mp[x] << " ";
            }
            std::cout << ": ";
            for (auto x : g_token.select[l][i]) {
                std::cout << (x == -1 ? "$" :  g_token.mp[x]) << " ";
            }
            std::cout << std::endl;
        }
    ...
    std::cout << (g_token.check_ll1() ? "LL1" : "Not LL1") << std::endl;
    std::cout << "\n";
    if (g_token.check_ll1()) {
        g_token.put_graph();
    }
    return 0;
} 